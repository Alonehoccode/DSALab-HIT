...
// ============================================================
#include <iostream> //Thư viện khai báo
#include <cmath>
#include <iomanip>
using namespace std;
 ...
 */
unsigned short TinhTongChuSo(unsigned int n )//có giá trị từ 0 → 4,294,967,295 dùng cho n vì 1 ≤ n ≤ 10^9
{
    unsigned short s = 0; //có giá trị từ 999999999= > tổng = 81
    while (n > 0) {
        s += n % 10; //chia lấy 1 số
        n /= 10; //xóa chưx số đó luôn
    }
    return s;
}
...
// ============================================================
#define SOMH 3 //hàng 0 cột 3 trong C sẽ là float a[][3]
#define MAX_SV 100 

struct SinhVien {
    int masv;
    char hoten[51];
    bool gtinh;     // true = Nam, false = Nu ( kiểm tra giá trị đúng sai nếu đúng trả về 1 nếu sai trả về 0)
    char lop[9]; 
    float dtb;
};
 ...
 *  O(2^N)     - mu
 */
void MinhHoaBigO() //xuất ra giá trị được minh họa
{
    int N = 8;
    long long dem = 0;
